...
//  List<Coin> listCoin = [Ethereum(),BitCoin()];
  ...
  void initState() {
    super.initState();
    AddressBook addressBook = Provider.of<AddressBook>(Global.context);
    addressBook.detailsAddress(widget.addressInfo.id);
    _addressController =
    ...
    if (widget.addressInfo != null) {
      ...
      new TextEditingController(text: widget.addressInfo.address);
      _remarkController =
      new TextEditingController(text: widget.addressInfo.description);
    } else {
      ...
      builder: (context, walletCenter, child) =>
          Container(
            child: Row(
              ...
              children: <Widget>[
                SizedBox(width: 15,),
                Text("选择币种", style: TextStyle(color: MyColors.BLACK_TEXT_22,
                    fontSize: 20,
                      ...
                      children: <Widget>[
                        Container(
                          ...
                          child: Row(
                            ...
                            children: <Widget>[
                              SizedBox(width: 13,),
                              Image.network(walletCenter.currentWalletCenter
                                  .icon, width: 24, height: 24,),
                              SizedBox(width: 10,),
                              Text(walletCenter.currentWalletCenter.name,
                                style: TextStyle(fontSize: 14,
                                    color: MyColors.BLACK_TEXT_22),),
                              SizedBox(width: 24,),
                              Image.asset("images/icon_coin_sel.png", width: 22,
                                height: 22,),
                            ],
                          ),
                        ),
          ...
          Row(
            children: <Widget>[
              ...
              InkWell(
                  ...
                  child: Image.asset(
                    "images/icon_scan_theme.png", width: 18, height: 18,)
              ),
            ],
          ),
          SizedBox(height: 15,),
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: "输入或粘贴钱包地址",
                hintStyle: TextStyle(
                    color: MyColors.GRAY_TEXT_99, fontSize: 12),
              ),
...
//  }
  ...
  Widget _buildSaveButton() {
    //判断是否可以点击按钮
    bool isPass = _addressController.text.isNotEmpty &&
        _remarkController.text.isNotEmpty;

    return Consumer<AddressBook>(
      builder: (context,addressBook,child) => Row(
        children: [
          BaseButton().getBaseButton(text: "删除",color: Color.fromARGB(0xff, 0xe5, 0x0f, 0x31),width: 160,height: 56, onPressed: ()async{


            await addressBook.delAddress(widget.addressInfo.id);
          }),
          BaseButton().getBaseButton(
              text: isPass ? "保存" : "请填写信息", width: 160,height: 56,onPressed: isPass ? () async {
