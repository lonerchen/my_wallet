...
//  }
  ...
  // 自定义转账
  // data的组成，
  // 0x + 每个要调用的合约方法的function signature + 要传递的方法参数，参数都为64位
  ...
  static Future<String> sendToken(String fromAddress, String toAddress,
      String num, Map token, Map obj,bool isEth) async {

    final hexNum = ((BigInt.from(double.parse(num))) * BigInt.from(pow(10, token['decimals'] ?? 16)))
        .toRadixString(16);
    String postData =
        '${func['transfer(address,uint256)']}${toAddress.replaceFirst("0x", "").padLeft(64, '0')}${hexNum.padLeft(64, '0')}';
    return await Http().transaction(fromAddress,toAddress,num,token, obj, postData,isEth:isEth);
  }
...
//    print(rsp.body);
  ...
  static Future approve(String token, Map obj) async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    int chainId = await prefs.getInt("chainId");
