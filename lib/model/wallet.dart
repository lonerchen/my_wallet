...
//        });
    ...
    print('hash   => ${this.currentWalletObject["hash"]}');
    ///
    /// 切换钱包对应的用户
    ///
    if(address != null) {
      await AccountToken.getAccountToken(currentWalletObject["hash"]);
      UserInfo userInfo = await Provider.of<UserInfo>(Global.context);
  ...
  Future<int> add(Map item,String pwd) async {
    ...
    //2.5将钱包的助记词hash传到服务器中
//    await DioUtils.getInstance().post(URL.createUser,params: {
//      "hash":HashCrypt().hash("${item['mnemonic']}"),
//      "address":address,
//      "coin":"ETH"
//    });

    // 3. 加密
    item['hash']       = await HashCrypt().hash("${item['mnemonic']}");
    item['privateKey'] = await WalletCrypt(pwd,item['privateKey']).encrypt();
    item['mnemonic']   = await WalletCrypt(pwd,item['mnemonic']).encrypt();

    var sql = SqlUtil.setTable("wallet");
    String sqlInsert ='INSERT INTO wallet(name,hash, mnemonic, privateKey, address, balance) VALUES(?, ? , ?, ?, ?, ?)';
    List list = [name,item['hash'],item['mnemonic'], item['privateKey'], address, balance];
